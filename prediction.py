...
class Predictor:
    """GCN-GAT predictor class.

    :param model: GCN-GAT model (pre-trained) used to forecast and reconstruct
    ...
    GCN-GAT 预测器类。
     ...
     :param model：GCN-GAT 模型（预训练）用于预测和重建
     :param window_size: 输入序列的长度
     :param n_features：输入特征的数量
     :param pred_args: 阈值和预测异常的参数
    ...
    def get_score(self, values):
        ...
        使用给定模型和数据计算异常分数的方法
        :param 值：多元时间序列数据的二维数组，形状 (N, k)
        ：返回 np 异常分数数组 + 数据帧，预测每个通道和全局异常
        """

        print("Predicting and calculating anomaly scores..")  #预测和计算异常分数
        ...
        with torch.no_grad():
            for x, y in tqdm(loader):
                ...
                # Shifting input to include the observed value (y) when doing the reconstruction 在进行重建时移动输入以包括观察值 (y)
                recon_x = torch.cat((x[:, 1:, :], y), dim=1)
                _, window_recon = self.model(recon_x)

                preds.append(y_hat.detach().cpu().numpy())
                # Extract last reconstruction only  仅提取上次重建
    ...
    def predict_anomalies(self, train, test, true_anomalies, load_scores=False, save_output=True,
        ...
        :param train：训练多元时间序列数据的二维数组
         :param test：测试多元时间序列数据的二维数组
         :param true_anomalies: 测试集的真实异常，如果不可用则无
         :param save_scores: 是否保存train和test的异常分数
         :param load_scores: 是否加载异常分数而不是计算它们
         :param save_output: 是否保存输出数据帧
         :param scale_scores: 是否按特征缩放异常分数
        """

        if load_scores:  #还没有执行这里
            ...
            train_pred_df = self.get_score(train)  #都的到了预测的异常分数
            test_pred_df = self.get_score(test)

            train_anomaly_scores = train_pred_df['A_Score_Global'].values
            test_anomaly_scores = test_pred_df['A_Score_Global'].values

            train_anomaly_scores = adjust_anomaly_scores(train_anomaly_scores, self.dataset, True, self.window_size)  #得到的异常分数 传给函数 查看原来的异常标签
        ...
        # Find threshold and predict anomalies at feature-level (for plotting and diagnosis purposes) 在特征级别查找阈值并预测异常（用于绘图和诊断目的）
        ...
        # 使用所有特征的异常分数聚合来预测全局异常（实体级别）
         # 这些预测用于评估性能，因为真正的异常在实体级别被标记
         # 使用不同的阈值方法进行评估：brute-force、epsilon 和 peaks-over-treshold
        ...
        # 保存使用 epsilon 方法进行的异常预测（可以更改为 pot 或 bf 方法）
        if save_output:
