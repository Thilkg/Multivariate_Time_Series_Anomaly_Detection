...
class Plotter:
    ...
    def create_shapes(self, ranges, sequence_type, _min, _max, plot_values, is_test=True, xref=None, yref=None):
        ...
        为区域创建形状以在图中突出显示（真实和预测的异常序列）。

         :param 范围：一个特征的异常序列的开始和结束索引的元组
         :param sequence_type: "predict" 如果预测值，否则 "true" 如果实际值。 确定颜色。
         :param _min: 系列的最小值
         :param _max: 系列的最大值
         :param plot_values: 要绘制的不同系列的字典

         :return: plotly 的形状规格列表
        """
