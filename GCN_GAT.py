...
class GCN_GAT(nn.Module):
    ...
    :param kernel_size: 在一维卷积中使用的内核大小
    :param feat_gat_embed_dim：嵌入维度（线性变换的输出维度）
           在面向功能的 GAT 层
    :param time_gat_embed_dim：嵌入维度（线性变换的输出维度）
           在面向时间的 GAT 层
    :param use_gatv2: 是否使用 GATv2 修改后的注意力机制而不是标准 GAT
    :param gru_n_layers: GRU 层的层数
    :param gru_hid_dim: GRU 层中的隐藏维度
    :param forecast_n_layers：基于FC的预测模型中的层数
    :param forecast_hid_dim：基于FC的预测模型中的隐藏维度
    :param recon_n_layers：基于 GRU 的重建模型中的层数
    :param recon_hid_dim：基于 GRU 的重建模型中的隐藏维度
    :param dropout: 辍学率
    :param alpha：泄漏依赖激活函数中使用的负斜率
    ...
    def forward(self, x):
        ...
        # x shape (b, n, k)：b - 批量大小，n - 窗口大小，k - 特征数
